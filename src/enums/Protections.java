...
import utils.MagicHook;
import utils.NBTStuff;

public enum Protections {

	
	DisableInWorlds(1,new String[] {}, "Disable In Worlds", "ALL", "Misc.DisableInWorlds", "Disables IllegalStack in the specified worlds.   Warning... this completely disables ALL protections in that world, meaning if a dupe exploit exists then it is up to you to make sure that players can not transfer items from a unprotected world, into a protected one!  Use at your own risk!","",0, false),
	//MISC SETTINGS
	InGameNotifications(3,true,"In Game Notifications", "ALL" , "Misc.InGameOffenseNotifications", "Send notifications to staff members in chat whenever an exploit is detected, this permission defaults to OP but can be granted by giving a player the: illegalstack.notify permission node. ","",0, false),
	LogOffensesInSeparateFile(3,true, "Logging Offenses To File", 3 , "Misc.LogOffensesInSeparateFile", "Records all offenses and locations caught by IllegalStack into a separate file located in plugins/IllegalStack/OffenseLog.txt","",0, false),
	PlayerOffenseNotifications(3, false, "Notify Nearby Players of Offenses", 3 , "Misc.NotifyNearbyPlayers", "Notify players in a 10 block radius of a offense or any that directly were caused by them", "",0, false),
	...
	BlockNonPlayersInNetherPortal(16,true,"Prevent Nether Portal Dupe", "ALL", "Exploits.PortalDupe.BlockNonPlayersInNetherPortal", "This setting prevents non player entities from travelling through a nether portal (typically horses and donkeys).  This exploit is used to duplicate the contents of a entitiys inventory by killing them just as they enter the portal, this setting also defeats sand / falling block dupers.","",0, false),
		NetherWhiteList(16,new String[] {},"Nether Portal Whitelist", 16, "Exploits.PortalDupe.NetherWhiteList", "Add any entities you would like to be able to travel to the nether.   Be warned if the creature is able to pick up items or have an inventory players WILL be able to dupe on your server.","",0, false),
		NetherWhiteListMode(16, true, "Whitelist Mode", 16, "Exploits.PortalDupe.NetherWhiteListMode", "If TRUE this list is a whitelist, if FALSE this list is a blacklist.","",0, false),
	BlockNonPlayersInEndPortal(17,true,"Prevent End Portal Dupe", "ALL", "Exploits.PortalDupe.BlockNonPlayersInEndPortal", "This setting prevents non player entities from travelling through the end portal (typically horses and donkeys).  This exploit is used to duplicate the contents of a entitiys inventory by killing them just as they enter the portal, this setting also defeats sand / falling block dupers.","",0, false),
		EndWhiteList(17,new String[] {},"End Portal Whitelist", 17, "Exploits.PortalDupe.EndWhiteList", "Add any entities you would like to be able to travel to the end.   Be warned if the creature is able to pick up items or have an inventory players WILL be able to dupe on your server.","",0, false),
		EndWhiteListMode(17, true, "Whitelist Mode", 17, "Exploits.PortalDupe.EndWhiteListMode", "If TRUE this list is a whitelist, if FALSE this list is a blacklist.","",0, false),
		NotifyBlockedPortalAttempts(17,false,"Notify Staff on Blocked Portal Attempts", 17, "Exploits.PortalDupe.NotifyBlockedPortalAttempts","Notify staff whenever a portal attempt is blocked, this option is OFF by default as sand duper setups can be very spammy.","",0, false),
	PreventEndGatewayCrashExploit(49, true, "Prevent End Gateway Crash Exploit", "> 1.9", "Exploits.EndGateway.PreventEndGatewayCrashExploit","Prevents players from riding entities through a end gateway, this can be used to crash the server.","",0, false),	
	PreventHoppersToUnloadedChunks(11,true,"Prevent Hopper Unload Exploit", "ALL", "Exploits.Other.PreventHoppersToUnloadedChunks", "Prevents a dupe glitch when two hoppers are used across a chunk border.","",0, false),
	PreventMinecartGlitch(15,true, "Prevent Minecart Glitching", "ALL", "Exploits.MineCart.PreventMinecartGlitch", "Prevents players using pistons to push a block or push a mincart into a block, this exploit is typically used in collection systems.","",0, false),
		MinecartBlockWhiteList(15,new String[] {}, "Allow Minecart Glitch Into", 15, "Exploits.MineCart.MinecartBlockWhiteList", "Any block types (Materials) added here will be ignored if a minecart is glitched into them.","",0, false),
		RemoveExistingGlitchedMinecarts(15,false, "Remove Old Glitched Minecarts", 15, "Exploits.MineCart.RemoveExistingGlitchedMinecarts", "This setting will detect minecarts that have been previously glitched into a block, It is recommended to only turn this setting on if you know players have glitched minecarts in your worlds, once you're sure it's been removed turn this feature back off for performance reasons.","",0, false),
	KickForAutoClickerFishing(31, true, "Prevent Auto Clicker Fishing", "ALL", "Exploits.Fishing.KickForAutoClickerFishing", "Detects autoclickers that spam fish using a block where the player spams the right mouse button (usually automatically) and fish are reeled in the instant they bite.","",0, false),
	WatchForAutoFishMod(20,true, "Prevent Auto Fish Mods", "ALL", "Exploits.FishMod.WatchForAutoFishMod", "Detects automatic fishing mods that cast and reel in fish that mimic regular player fishing, this is not an instantaneous detection and does not detect auto fishing on the first attempt..","",0, false),
		MaxFishAllowedBeforeKick(20,5,"Max fish before kick", 20,"Exploits.FishMod.MaxFishAllowedBeforeKick","This is the number of fish a player is allowed to catch before they are kicked (once detected for auto fishing)","",0, false),
		WarnPlayerThenKickInsteadOfNotify(20,false,"Notify Instead of Kick", 20, "Exploits.FishMod.WarnPlayerThenKickInsteadOfNotify", "This setting defaults to false, and staff is notified when a player is suspected of using an auto fishing mod.  If set to true the player will be warned right before they're about to be kicked after the plugin has detected they are using an auto-fishing mod.","",0, false),
		MaxFishToNotifyStaffThenBlock(20,5,"Max fish before notify", 20, "Exploits.FishMod.MaxFishToNotifyStaffThenBlock", "If Notify instead of kick is set to true, then staff will be notified once the plugin has detected a player using an auto fish mod after they catch this many fish.","",0, false),
	
	PreventIndirectTNTPowerDupe(21, true, "Prevent Tnt Dupers", "ALL", "Exploits.TNTDupe.PreventIndirectTNTPowerDupe", "Prevents tnt duping methods that exploit an indrect power bug that causes tnt to ignite and fall away / be launched but leave an unlit block of tnt behind.","",0, false),
	...
	RemoveOverstackedItems(6,true,"Remove Overstacked Items", "ALL", "Exploits.OverStack.RemoveOverstackedItems", "Detects and removes items that have amounts larger than the vanilla stack size.","",0, false),
		IllegalStackMode(6, true, "Whitelist Mode", 6, "Exploits.OverStack.StackWhiteListMode", "If TRUE this list is a whitelist, if FALSE this list is a blacklist.","",0, false),
		AllowStack(6,new String[] {"POTION"}, "Overstackable Items", 6, "Exploits.OverStack.AllowStack", "Items added to this list can be larger than the vanilla stack sizes, ie Potions/Ender Pearls","",0, false),
		PreventOverStackedItemInHoppers(6,true,"Remove Overstacked Items in Hoppers", 6, "Exploits.OverStack.PreventOverStackedItemInHoppers", "Removes overstacked items if they are found inside hoppers, this can be turned off safely if you do not have players with large stored amounts of overstacked items... If off players can use hoppers to extract items from a large stack one by one.","",0, false),
		FixOverstackedItemInstead(6,false,"Fix Illegal Stacks", 6, "Exploits.OverStack.FixOverstackedItemInstead", "Instead of removing the entire stack, set the stack to the maximum stack size for that item type.","",0, false),
		...
		FixIllegalEnchantmentLevels(25,false,"Fix Illegal Enchantment Levels", "ALL", "Exploits.Enchants.FixIllegalEnchants", "Will correct any enchantment level found that is larger than vanilla minecraft allows.", "",0, false),
			...
			AllowBypass(25, false, "Allow Permission Bypass", 25, "Exploits.Enchants.AllowBypass", "Will allow any player with the illegalstack.enchantbypass permission to bypass the enchantment level check (defaults to OP)..  Note if a player is given the item and does not have the permission it's enchantments will still be removed.","",2, false),
			RemoveUnbreakableFlag(25, false, "Remove Unbreakable flag from items", 25, "Exploits.Enchants.RemoveUnbreakableFlag", "Will remove the Unbreakable flag from items held by players (unless they have the bypass permission).", "", 0, false),
			RemoveCustomAttributes(25, false,"Remove Custom Attribute Modifiers", 25, "Exploits.Enchants.RemoveCustomAttributes", "Will remove ALL custom attributes found on items held by players (unless they have the bypass permission).  This is useful for getting rid of sticks with +1000 damage or insta death helmets that have been previously cheated in.  *NOTE*  If you're running < 1.13 this protection requires NbtAPI 2.1.0+ ","",2, false),
			DestroyIllegallyEnchantedItemsInstead(25,false,"Destroy Items Instead of Fixing", 25, "Exploits.Enchants.DestroyIllegallyEnchantedItemsInstead", "Instead of fixing the item destroy it instead", "", 0 , false),
	...
	PreventItemSwapLagExploit(37,true, "Prevent Item Swap Lag Exploit", "> 1.9", "Exploits.Other.PreventItemSwapLagExploit", "Prevents players from spamming the server with held item swaps creating lag on the server.","",0, false),
	...
	PreventCommandsInBed(47,true, "Prevent Commands While In Bed", "ALL", "Exploits.Other.PreventCommandsInBed","Prevent players from being able to use commands while in bed.  This has been linked to a serious exploit where not all events fire properly while a player is in bed, one huge exploit with this is players can get any item out of a GUI if they can open it while sleeping.","",0, false),
	...
	ResetSpawnersOfType(33, new String[] {}, "Force Spawner Reset", "1.13/1.14", "UserRequested.Spawners.ResetSpawnersOfType", "Resets a spawner of a given <Entity_Type> to a pig spawner if mined, only really useful if you have a silk spawner plugin and some specific spawners in the world that you don't want to allow players to mine.", "", 2, false),
	
	RemoveItemsMatchingName(23,false,"Remove Items With Specific Names", "ALL", "UserRequested.ItemRemoval.RemoveItemsMatchingName", "If this setting is set to TRUE, any item matching the name will be destroyed, useful if you have items taken out of GUI's due to another plugins bug.   OFF by default (User Requested Feature)","",2, false),
		ItemNamesToRemove(23,new String[] {}, "Item Names To Match", 23, "UserRequested.ItemRemoval.ItemNamesToRemove", "Add item names to this list and if RemoveItemsMatchingName is true, they will be removed like an illegal stacked item would be.","",2, false),
		ItemLoresToRemove(23,new String[] {}, "Item Lores To Match", 23, "UserRequested.ItemRemoval.ItemLoresToRemove", "Add any lores that identify items you wish to have removed, such as if players were able to get items out of a shop plugin illegally and IllegalStack will remove these items once detected.","",2, false),
		NameLoreStrictMatchMode(23,false, "Match lore | name exactly", 23, "UserRequested.ItemRemoval.NameLoreStrictMatchMode", "If this value is true, then the item name or lore line must EXACTLY match including color codes, when set to false as long as the text is included in the lore or name a match will be detected.","",2, false),
		BlockEnchantingInstead(23,false,"Block Enchants Instead", 23, "UserRequested.ItemRemoval.BlockEnchantingInstead", "If this value is true then instead of removing the named/lore matched item prevent players from enchanting this item.","",2, false),
		BlockRepairsInstead(23,false,"Block Repairs Instead", 23, "UserRequested.ItemRemoval.BlockRepairsInstead", "If this value is true then instead of removing the named/lore matched item prevent players from repairing this item.","",2, false),

	NotifyInsteadOfBlockExploits(26,new String[] {}, "Notify ONLY Instead Of Block", "ALL", "UserRequested.Misc.NotifyInsteadOfBlock", "Any protections added to this list will NOT be blocked, however notifications will still be sent.   Caution: This may produce a fair amount of spam for some protections","",2, false),
	BlockBadItemsFromCreativeTab(30,false,"Block Bad Items From Creative Tab", "> 1.12", "UserRequested.Obscure.BlockBadItemsFromCreativeTab", "Prevents players from giving themselves items in a single player world with metadata into a server if they have access to GMC.   This exploit allows players to create items not normally obtainable through regular /GMC such as sticks that give 5000 health when held... NOTE: this fix requires protocollib to be installed to work!", "",2, false),
	DestroyInvalidShulkers(31,false,"Destroy Client Crashing Shulkers", "> 1.11", "UserRequested.Obscure.HackedShulker.DestroyInvalidShulkers", "Destroys Shulker boxes created by hacked clients that are used to ban players (Book ban variation) **NOTE** Requires NBT API 2.0.0 (in all spigot versions) to work!!", "",2, false),
		CheckGroundForBadShulkerAtLogin(31,false,"Remove Bad Shulkers on Ground", 31, "UserRequested.Obscure.HackedShulker.CheckGroundForBadShulkerAtLogin","Removes any dropped shulker boxes in the world found when a player logs in that contain invalid client crashing data.  You should ONLY enable this protection if you know you have areas with bad shulkers in a chunk.  This protection will turn itself off each time the server restarts. **NOTE** Requires NBT API 2.0.0 to work!!","",2, false),
	...
	public Object getConfigValue() {
		if(isList)
			return (HashSet<String>) getTxtSet();
	...
	public Object getDefaultValue() {
		...
		if(defaultValue instanceof HashSet) 
			return (HashSet<String>) defaultValue;
	...
	Protections (int id, String[] array, String dname, Object ver, String path, String desc, String cmd, int catId, boolean relevant) {
		...
		for(int i = 0; i < array.length; i++)
			((HashSet<String>)this.defaultValue).add(array[i]);
		...
		for(int i = 0; i < array.length; i++) 
			this.getTxtSet().add(array[i]);
		
		
		setBasics(id,ver,dname,desc,path, cmd);
		
	}
	Protections (int id, Integer intVal, String dname, Object ver, String path, String desc, String cmd, int catId, boolean relevant) {
		this.defaultValue = intVal;
		this.setCatId(catId);
		this.setIntValue(intVal);
		setBasics(id,ver,dname,desc,path,cmd);
	}
	Protections (int id, Boolean value, String dname, Object ver, String path, String desc,String cmd, int catId, boolean relevant) {
	...
	Protections (int id, String setting, String dname, Object ver, String path, String desc,String cmd, int catId, boolean relevant) {
	...
	public boolean isRelevantToVersion(String serverVersion) {
		...
		if(this.getVersion().contains("< 1.13"))
			if(serverVersion.contains("1.14") || serverVersion.contains("1.15"))
				return false;
			else
				return true;
	...
	public boolean isVersionSpecific(String serverVersion) {
		...
		if(this.getVersion().equals("1.15")) { 
			if(serverVersion.contains("v1_15_R1"))
				return true;
			else if(serverVersion.contains("v1_15_R2"))
					return true;
				return false;
		}
		if(this.getVersion().equals("1.14.4")) { 
			if(serverVersion.contains("v1_14_R4"))
				return true;
			else
				return false;
		}
		...
		if(this.getVersion().equals("1.14.3")) {
			if(serverVersion.contains("v1_14_R3")) 
				return true;
			else
				return false;
		}
		...
		if(serverVersion.contains("v1_9") && this.getVersion().contains("1.9"))
			return true;
		if(serverVersion.contains("v1_8") && this.getVersion().contains("1.8"))
			return true;
	...
	public boolean validate(String value, CommandSender sender) {
		...
		*/
		...
		if(this == Protections.LimitNumberOfPages || this == Protections.NetherYLevel || this == Protections.VillagerRestockTime || this == Protections.ZombieVillagerTransformChance || this == Protections.PageCountThreshold || this == MaxFishAllowedBeforeKick || this == MaxFishToNotifyStaffThenBlock ) {
			try {
				Integer intCheck = Integer.parseInt(value.trim());
				if(intCheck < 0) {
	...
	public boolean loreNameMatch(ItemMeta im) {
		...
		for(String s:target.keySet()) {
			...
			if(target.get(s) && im.hasLore()) //lore matching
			{
				...
				if(Protections.NameLoreStrictMatchMode.isEnabled()) {
					if(im.hasDisplayName() && im.getDisplayName().equals(s))
						found = true;
				} else if(im.hasDisplayName() && ChatColor.stripColor(im.getDisplayName()).contains(s))
						found = true;
			}
		}
		return found;
	...
	public boolean isThirdPartyInventory(InventoryView inv) {
		...
		if(IllegalStack.getPlugin().getServer().getPluginManager().getPlugin("DynamicShop") != null) {
			String name = LangUtil.ccLang.get().getString("TRADE_TITLE");
		...
		if(IllegalStack.getPlugin().getServer().getPluginManager().getPlugin("ShopGUIPlus") != null) {
			if(inv.getTopInventory().getHolder() instanceof BInventoryHolder)
				return true;
	...
	public boolean isAllowedEnchant(Enchantment en,int lvl) {
		...
		for(String s:this.getTxtSet())
		{
			...
			if(lvl > level)  //level higher than override.
				return false;
	...
	public static void fixEnchants(Player player) {
		...
		ItemStack is = player.getInventory().getItemInMainHand();
			NBTStuff.checkForBadCustomData(is,player,true);
			if(is == null) {
			...
			for(Enchantment en:is.getEnchantments().keySet()) 
				if(is.getEnchantmentLevel(en) > en.getMaxLevel())
				{
					...
					if(en.canEnchantItem(is))
						player.sendMessage(Msg.IllegalEnchantLevel.getValue(player,is,en));
					else 
						player.sendMessage(Msg.IllegalEnchantType.getValue(player, is, en));
					replace.add(en);
			...
			for(Enchantment en:replace) {
				...
				if(en.canEnchantItem(is)) {
					...
					player.sendMessage(Msg.StaffEnchantFixed.getValue(player, is, en));
				}
			}
			if(replace.isEmpty())
